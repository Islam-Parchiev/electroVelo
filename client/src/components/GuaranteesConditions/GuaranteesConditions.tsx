...
interface GuaranteesConditionsProps {}

const GuaranteesConditions: React.FC<GuaranteesConditionsProps> = props => {
	const {} = props
	return (
			...
			<div className="container">
				...
				<ul className={`list-reset ${styles.GuaranteesConditions__list}`}>
					<li className={styles.GuaranteesConditions__item}>
						правильно и без помарок и исправлений заполненный гарантийный талон,
						 в котором должны быть указаны модель и серийный номер изделия, дата продажи и печать торгующей организации;
					</li>
					<li className={styles.GuaranteesConditions__item}>
				...
				<div className={styles.GuaranteesConditions__text}>
					<p>Обращаем также ваше внимание на то, что при получении и оплате заказа покупатель
						 в присутствии курьера обязан проверить комплектацию и внешний вид изделия на предмет 
						 отсутствия физических дефектов (царапин, трещин, сколов и т.п.) и полноту комплектации.
